...
const RSP = () => {
  ...
  const resultHandler = (mode_1, mode_2) => {
    if (mode_1 === mode_2) setResult({ str: "draw" });
      ...
      setResult({ str: "win" });
    else if (mode_1 === "paper" && mode_2 === "rock") setResult({ str: "win" });
  ...
  const betHandler = (gold) => {
    if (!bet.isBet) {
      setBet((prevState) => ({ ...prevState, isBet: true, amount: gold }));
    } else {
  ...
  // since consecutive wins or losses aren't captured by React, include the state as object in the dependency array.
